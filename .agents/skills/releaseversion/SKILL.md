...
allowed-tools: Read, Edit, Grep, WebFetch, Bash(flutter:*), Bash(git add:*), Bash(git branch:*), Bash(git commit:*), Bash(git describe:*), Bash(git diff:*), Bash(git fetch:*), Bash(git log:*), Bash(git pull:*), Bash(git push:*), Bash(git status:*), Bash(git switch:*), Bash(git tag:*), Bash(gh:*)
---
...
Follow these steps in order. Do not skip the constraints at the bottom. Never commit, tag,
push, build a signed artifact, open a PR, or create a GitHub release until the applicable
confirmation gate has been approved.
...
- Before doing anything else, return to the user with the branch plan (which branch becomes
  the PR head) and the fact that this cuts a **PR into `main` by default** — never push to
  `main` directly unless `--push` was explicitly given in `$ARGUMENTS` or requested earlier in
  the conversation. Use `functions.request_user_input` when available; otherwise stop and wait
  for a normal chat reply. Do not create branches, build, or push in the same turn. If
  declined, stop.
...
- Use `functions.request_user_input` (or a normal chat reply if unavailable) to ask: has the
  PR been merged into `main` using "Create a merge commit", and are the release notes approved
  as printed (or with edits)? Do not proceed until confirmed — this may span an arbitrary
  amount of real time while the PR is reviewed.
